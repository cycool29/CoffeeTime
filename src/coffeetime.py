...
def coffee_break_countdown():
    ...
    current_time_seconds = int(datetime.datetime.now().strftime("%H")) * 3600 + int(
        datetime.datetime.now().strftime("%M")) * 60  # hour to seconds + minutes to seconds
...
class MainWindow:
    ...
    def __init__(self) -> None:
        ...
        self.quote_label = tk.Label(master=self.time_frame,
        ...
        self.time_spinbox = tk.Spinbox(master=self.spinbox_frame, from_=0, to=10000, font=default_font, width=12,
        ...
        self.start_button = tk.Button(master=self.start_frame, text='Start', command=start_coffee_break_countdown,
                                      font=default_font)
        ...
        self.settings_button = tk.Button(master=self.info_frame, text="Settings", font=default_font_name + ' 11',
...
class SettingsWindow:
    def launch_window(self):
        self.settings_window.geometry(
            "450x650+" + str(int(screen_width / 2 - 225)) + "+" + str(int(screen_height / 2 - 325)))
    ...
    def __init__(self) -> None:
        ...
        self.coffee_break_interval_label = tk.Label(master=self.coffee_break_interval_frame,
                                                    text='Coffee break interval:', font=default_font)
        self.coffee_break_interval_spinbox = tk.Spinbox(master=self.coffee_break_interval_frame, from_=1, to=10000,
                                                        textvariable=self.coffee_break_interval_stringvar,
                                                        font=default_font, command=self.update_config)

        self.coffee_break_message_frame = tk.Frame(master=self.settings_window)
        self.coffee_break_message_label = tk.Label(master=self.coffee_break_message_frame,
                                                   text='\nCoffee break message:', font=default_font)
        self.coffee_break_message_entry = tk.Entry(master=self.coffee_break_message_frame,
                                                   textvariable=self.coffee_break_message_stringvar, font=default_font,
                                                   validatecommand=self.update_config)

        self.coffee_break_sound_frame = tk.Frame(master=self.settings_window)
        self.coffee_break_sound_label = tk.Label(master=self.coffee_break_sound_frame,
                                                 text='\nSelect a coffee break sound effect:', font=default_font)
        self.coffee_break_sound_button = tk.Button(master=self.coffee_break_sound_frame, text='Select file',
                                                   font=default_font, command=self.choose_sound_file)
        ...
        self.theme_spinbox = tk.Spinbox(master=self.theme_frame, textvariable=self.theme_stringvar, font=default_font,
                                        command=self.change_theme, values=self.theme_values, wrap=True)
...
class NotificationWindow:
    ...
    def launch_window(self):
        ...
        self.notification_window.geometry(
            "300x100+" + str(int(screen_width / 2 - 150)) + "+" + str(int(screen_height / 2 - 50)))
...
def system_tray_icon():
    ...
    tray = sg.SystemTray(menu=menu_def, filename='/home/pi/coffee-reminder/src/icon.png')
    while True:
